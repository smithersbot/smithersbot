...
#
# Suggested cron: */30 * * * * ~/moltbot/scripts/auth-monitor.sh
#
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Moltbot is down. Run: ssh user@gateway-host '~/moltbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
