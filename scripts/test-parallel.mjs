...
export function isGoalTestScopeEnabled(env = process.env) {
  return env.MOLTBOT_GOAL_TEST_SCOPE === "1";
}

export function resolveRuns(env = process.env) {
  return isGoalTestScopeEnabled(env) ? GOAL_SCOPED_RUNS : DEFAULT_RUNS;
}

export async function runTestParallel(env = process.env) {
  const runs = resolveRuns(env);
  const children = new Set();
  const isCI = env.CI === "true" || env.GITHUB_ACTIONS === "true";
  const isMacOS = process.platform === "darwin" || env.RUNNER_OS === "macOS";
  const isWindows = process.platform === "win32" || env.RUNNER_OS === "Windows";
  const isWindowsCi = isCI && isWindows;
  const shardOverride = Number.parseInt(env.CLAWDBOT_TEST_SHARDS ?? "", 10);
  const shardCount =
    isWindowsCi ? (Number.isFinite(shardOverride) && shardOverride > 1 ? shardOverride : 2) : 1;
  const windowsCiArgs = isWindowsCi
    ? ["--no-file-parallelism", "--dangerouslyIgnoreUnhandledErrors"]
    : [];
  const overrideWorkers = Number.parseInt(env.CLAWDBOT_TEST_WORKERS ?? "", 10);
...
if (isDirectExecution()) {
  const code = await runTestParallel();
  process.exit(code);
}
