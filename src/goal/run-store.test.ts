  ...
  it("round-trips build-gate and ralph tracking fields", () => {
    const session: GoalSession = {
      ...
      buildGateFixSignatures: {
        "step-1":
          "semgrep scan --config auto --error --quiet --severity ERROR --timeout 30 --exclude 'node_modules' --exclude 'dist' --exclude '.git' --exclude '.next' --exclude 'build' --exclude '*.test.ts' src/example.ts\npnpm build",
      },
      buildGateResults: {
        "step-1": {
          passed: false,
