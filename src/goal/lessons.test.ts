...
describe("extractRunLessons", () => {
  ...
  it("stays fail-open when all lesson backends fail with the anthropic auth error", async () => {
    const runId = "extract-run-anthropic-auth";
    const workingDir = "/repo/project-anthropic-auth";
    saveExtractionRun({
      runId,
      workingDir,
      stepResults: {
        "step-alpha": {
          stepId: "step-alpha",
          success: false,
          output: "needs corrections",
          error: "force corrections so extraction runs",
          durationMs: 50,
        },
      },
    });
    ...
    mockRunCliProcess
      .mockResolvedValueOnce(makeCliResult({ stdout: "", stderr: authStderr, exitCode: 1 }))
      .mockResolvedValueOnce(makeCliResult({ stdout: "", stderr: authStderr, exitCode: 1 }));

    let recorded: Awaited<ReturnType<typeof extractRunLessons>> | undefined;
    await expect(
      (async () => {
        recorded = await extractRunLessons(runId, workingDir, []);
      })(),
