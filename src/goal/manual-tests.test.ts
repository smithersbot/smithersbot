...
describe("generateManualTests", () => {
  ...
  it("parses model suggestions and formats criticality in range", async () => {
    ...
    const { tests, status } = await generateManualTests({
  ...
  it("returns deterministic fallback when GoalLlmClient throws the anthropic auth error", async () => {
    const authMessage =
    ...
    const client: GoalLlmClient = {
      complete: vi.fn().mockRejectedValue(new Error(authMessage)),
    };

    const result = await generateManualTests({
      goal: "Improve authentication reliability",
      steps: makeDoneSteps(),
      client,
    });

    expect(result.status).toBe("skipped_no_embedded_auth");
    expect(result.tests.length).toBeGreaterThan(0);
    expect(result.tests[0]?.reason).toContain("Embedded-agent auth unavailable");
    expect(result.tests[0]?.detail).toContain("**Step 1.**");
    expect(result.error).toBeDefined();
    expect(result.error).not.toContain("/agentDir");
    expect(result.error).not.toContain("auth-profiles.json");
    expect(result.error).toMatch(/embedded-agent auth unavailable/i);
  ...
  it("retries once when the first response is unparseable and then succeeds", async () => {
    ...
    try {
      ...
      const { tests: manualTests } = await pending;
  ...
  it("uses Codex-only manual test generation when Claude Code is unavailable", async () => {
    ...
    const { tests: manualTests } = await withNonTestEnv(() =>
      generateManualTests({
  ...
  it("returns an empty array when the model explicitly returns tests: []", async () => {
    ...
    const { tests: manualTests } = await generateManualTests({
      goal: "Improve authentication reliability",
      steps: makeDoneSteps(),
      client,
    });
