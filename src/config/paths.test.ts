...
describe("state + config path candidates", () => {
  ...
  it("managed root defaults to ~/smithersbot-home under home", () => {
    const env = {} as NodeJS.ProcessEnv;
    const home = "/home/test-user";
    expect(resolveManagedRoot(env, () => home)).toBe(path.join(home, DEFAULT_MANAGED_ROOT_DIRNAME));
  });

  it("managed root respects an existing former default when the new default is absent", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "smithersbot-managed-root-"));
    try {
      const legacyRoot = path.join(root, "smithersbot-goals");
      await fs.mkdir(legacyRoot);
      expect(resolveManagedRoot({} as NodeJS.ProcessEnv, () => root)).toBe(legacyRoot);
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("agent root is <managed>/agent and workspace repo lives inside it", () => {
    ...
    expect(repo).toBe(path.resolve("/m/root/agent/workspaces/smithersbot"));
  ...
  it("setup-smithersbot.sh MANAGED_ROOT_SUBDIRS matches the resolver layout", () => {
    ...
    const workspacesParent = path.dirname(resolveWorkspaceRepoDir("ws", env, home));
