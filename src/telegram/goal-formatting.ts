...
const MANUAL_TESTS_SKIPPED_NO_EMBEDDED_AUTH_NOTICE =
  "Manual test generation used a deterministic fallback: embedded-agent auth unavailable.";
...
export function appendManualTestsStatusNotice(
  ...
  if (status === "skipped_no_embedded_auth") {
    if (summary.includes(MANUAL_TESTS_SKIPPED_NO_EMBEDDED_AUTH_NOTICE)) return summary;
    return `${summary.trimEnd()}\n${MANUAL_TESTS_SKIPPED_NO_EMBEDDED_AUTH_NOTICE}`;
  }
