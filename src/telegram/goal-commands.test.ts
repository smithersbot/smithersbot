...
describe("goal-commands telegram adapter", () => {
  ...
  describe("handleGoalApprove", () => {
    ...
    it("surfaces the codex-network message when backend=codex and resume hits a connection failure", async () => {
      saveRunFixture(makeRun({ plannerBackendUsed: "codex" }));
      mockGoalResumeCommand.mockRejectedValueOnce(
        new Error("fetch failed: ECONNRESET while talking to Codex"),
      );
      ...
      expect(typeof result).toBe("string");
      expect(result).toContain("Planning failed: Couldn't connect to Codex.");
      expect(result).toContain("Please check your network and try:");
      expect(result).toContain("sudo npm install -g @openai/codex@latest");
      expect(result).toContain("codex login");
      expect(result).toContain('codex "say only: codex works"');
    });
  });
  ...
  describe("handleGoalStatus", () => {
    ...
    it("surfaces completed-run delivery and GitHub fields through /goal_status", async () => {
      const reviewUrl =
        "https://github.com/smithers/test-private/tree/smithersbot/20260525-120000Z-goal-status";
      saveRunFixture(
        makeRun({
          runId: "status-delivery-1234",
          state: "done",
          completionSummary: "✅ Done: Status delivery\n**Progress** 1/1\n**Goal ID:** status-d",
          deliveryFailed: true,
          deliveryError: "message unavailable",
          githubPushOutcome: {
            enabled: true,
            branch: "smithersbot/20260525-120000Z-goal-status",
            remote: "origin",
            attempted: true,
            succeeded: true,
      ...
      mockGoalStatusCommand.mockImplementationOnce(async (id, opts, runtime) => {
        const actual = await vi.importActual<typeof import("../commands/goal-status.js")>(
          "../commands/goal-status.js",
        );
        await actual.goalStatusCommand(
          id as string,
          opts as Parameters<typeof actual.goalStatusCommand>[1],
          runtime as Parameters<typeof actual.goalStatusCommand>[2],
        );
      });

      const { handleGoalStatus } = await import("./goal-commands.js");
      const result = await handleGoalStatus("status-delivery");

      expect(result).toContain("**Completion** ✅ Done: Status delivery");
      expect(result).toContain(reviewUrl);
      expect(result).toContain("smithersbot/20260525-120000Z-goal-status");
      expect(result).toContain("**Delivery** Completion notification failed: message unavailable");
    });
  });
  ...
  describe("registerTelegramGoalCommands /goal_github_push", () => {
    ...
    it("shows current GitHub push mode when no argument is provided", async () => {
      ...
      expect(sentText).toContain("links to the pushed branch for review");
      expect(sentText).toContain("does not automatically create pull requests");
