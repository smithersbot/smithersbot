...
- `src/cli/program/command-registry.ts` (eager top-level registrations + routed
  `RouteSpec`s).
- `src/cli/program/register.subclis.ts` (lazy `entries[]`; each is rendered as a
  placeholder by `registerSubCliCommands` and replaced on first invocation).
- `src/cli/program/help.ts` (`EXAMPLES` array shown in `--help`).
- Test references: `src/cli/program/register.subclis.test.ts`,
  `src/cli/program.smoke.test.ts`, `src/cli/run-main.test.ts`.

v0 CLI surface = `gateway`, `logs`, `system`, `models`, `approvals`, `cron`,
`skills`, `goal`, `setup`, `onboard`, `configure`, `memory`, `agent`. Delete-now
targets from the task brief = ACP, TUI, browser, plugins/channels, docs,
hooks/webhooks, node/canvas, daemon/deploy, legacy general-assistant. (Grep for
`general-assistant`, `registerGeneralAssistant`, `general_assistant` across
`src/` returns zero hits — no legacy general-assistant CLI surface remains; it
was already removed in earlier stages.)

### 9a. Eager registrations (`src/cli/program/command-registry.ts`)

| PATH (command id @ registrar) | IMPORTERS | PACKAGE/WORKSPACE/VITEST REFS | TESTS | REQUIRED BY v0? | DECISION | VERIFICATION NEEDED |
| --- | --- | --- | --- | --- | --- | --- |
| `setup` @ `src/cli/program/register.setup.ts` → `commands/setup.ts` + `commands/onboard.ts` | `command-registry.ts` L17,L109-111 | n/a (top-level command, no scripts/files entry) | `src/cli/program.smoke.test.ts` L131-145 (`runs setup without wizard flags`, `runs setup wizard when wizard flags are present`) | yes — v0 keeper (operator workflow) | keep | `pnpm vitest run src/cli/program.smoke.test.ts` |
| `onboard` @ `src/cli/program/register.onboard.ts` → `commands/onboard.ts` (full wizard) | `command-registry.ts` L18,L112-114; help examples in `help.ts` do not reference `onboard` directly but smoke test exercises it | n/a | `src/cli/program.smoke.test.ts` L147-203 (`passes auth api keys to onboard`) | yes — v0 keeper | keep | `pnpm vitest run src/cli/program.smoke.test.ts` |
| `configure` @ `src/cli/program/register.configure.ts` → `commands/configure.ts` | `command-registry.ts` L14,L116-119 | n/a | none direct (`configureCommand` mock asserted via the `config` alias smoke test L125-129) | yes — v0 keeper | keep | `pnpm vitest run src/cli/program.smoke.test.ts` |
| `config` @ `src/cli/config-cli.ts` (`registerConfigCli`, top-level `config` with `get`/`set`/`unset` subcommands + naked wizard alias dispatching to `configureCommand`) | `command-registry.ts` L12,L120-123 | `package.json` `files[]` ships `dist/cli/**` (covers all `src/cli/*.ts`); no scripts entry | `src/cli/program.smoke.test.ts` L125-129 (`runs config alias as configure`) | yes — paired with `configure` (same wizard) and provides v0 dot-path get/set used by operator | keep | `pnpm vitest run src/cli/program.smoke.test.ts` |
| `doctor`, `dashboard`, `reset`, `uninstall` @ `src/cli/program/register.maintenance.ts` (`registerMaintenanceCommands`) | `command-registry.ts` L15,L124-127 | n/a | `src/commands/*` tests for `doctor`, `reset`, `uninstall`; no direct CLI-registration test | partial — `doctor`/`reset`/`uninstall` are still useful for v0 operator support; `dashboard` opens Control UI (paired with Track I `ui/` decision) | keep `doctor`/`reset`/`uninstall`; quarantine-or-defer `dashboard` (depends on Track I `ui/` outcome) | `pnpm vitest run src/commands/doctor*.test.ts src/commands/reset*.test.ts src/commands/uninstall*.test.ts` after any registration edit |
| `message` @ `src/cli/program/register.message.ts` (top-level `message` + subcommands `send`, `broadcast`, `poll`, `react`, `reactions`, `read`, `edit`, `delete`, `pin`, `unpin`, `pins`, `permissions`, `search`, `thread create/list/reply`, `emoji list/upload`, `sticker send/upload`) | `command-registry.ts` L16,L128-131; help `EXAMPLES` in `help.ts` L11,L13,L26 references `message send/react` with `--channel telegram` and `--channel signal`/non-Telegram targets | `package.json` `files[]` ships `dist/cli/**` | `src/cli/program.smoke.test.ts` L62-90 (`runs message with required options`, `runs message react with signal author fields`) | no — v0 outbound is Telegram-only; `message` is the cross-channel send/reaction/thread surface (Signal, Discord, Slack, WhatsApp targets included in tests) | quarantine-or-defer for Track F/H (hide registration; deletion couples with Track K legacy auto-reply cleanup and `src/auto-reply/` decision) | `pnpm vitest run src/cli/program.smoke.test.ts src/cli/program/message/` after any change |
| `memory` @ `src/cli/memory-cli.ts` (`registerMemoryCli`); also routes `memory status` via `routeMemoryStatus` in command-registry | `command-registry.ts` L13,L132-136 (with `routeMemoryStatus` L93-105) | `package.json` `files[]` ships `dist/cli/**`; `src/cli/memory-cli.test.ts` is in vitest default include | `src/cli/memory-cli.test.ts`; `src/cli/program.smoke.test.ts` L98-102 (`registers memory command`) | yes — v0 keeper (goal lessons/memory) | keep | `pnpm vitest run src/cli/memory-cli.test.ts src/cli/program.smoke.test.ts` |
| `agent`, `agents` (with `agents list/add/set-identity/delete`) @ `src/cli/program/register.agent.ts` (with `routeAgentsList` L83-91) | `command-registry.ts` L19,L137-142; help `EXAMPLES` references `moltbot agent --to ... --message ... --deliver` (L21-23) | n/a | no dedicated registration test; agents commands covered by `src/commands/agents*.test.ts` | yes — v0 keeper | keep | `pnpm vitest run src/commands/agents*.test.ts` |
| `subclis` (lazy fan-out) @ `src/cli/program/register.subclis.ts` (`registerSubCliCommands`) | `command-registry.ts` L20,L143-146 | n/a | `src/cli/program/register.subclis.test.ts` (asserts placeholder + lazy dispatch for `acp`, `nodes`) | yes — infrastructure; underlying entries decided in 9b | keep (mechanism); per-entry decisions in 9b | `pnpm vitest run src/cli/program/register.subclis.test.ts` |
| `status`, `health`, `sessions` @ `src/cli/program/register.status-health-sessions.ts` (with `routeStatus`/`routeHealth`/`routeSessions` L41-81 in command-registry) | `command-registry.ts` L21,L147-151 | n/a | `src/cli/program.smoke.test.ts` L92-96 (`runs status command`) | yes — v0 keeper (operator support / Telegram `/status`-adjacent flows) | keep | `pnpm vitest run src/cli/program.smoke.test.ts` |
| `browser` @ `src/cli/browser-cli.ts` (`registerBrowserCli`; fans out to `browser-cli-actions-input/observe`, `browser-cli-debug`, `browser-cli-examples`, `browser-cli-extension`, `browser-cli-inspect`, `browser-cli-manage`, `browser-cli-shared`, `browser-cli-state`) | `command-registry.ts` L9 (`import { registerBrowserCli } from "../browser-cli.js"`), L22,L152-155; help `EXAMPLES` does not list `browser` | `package.json` `files[]` ships `dist/cli/**`; assets shipped via `assets/**` | `src/cli/browser-cli.test.ts`, `src/cli/browser-cli-extension.test.ts`, `src/cli/browser-cli-inspect.test.ts` | no — browser CLI is not part of SmithersBot v0 (per task delete list: `browser`) | quarantine-or-defer (Track H comments the `command-registry.ts` L9 import + L152-155 registration with the Stage 2G hide note; defer file deletion until Track I paired call) | `pnpm vitest run src/cli/program/ src/cli/run-main.test.ts src/cli/browser-cli*.test.ts` |
| `goal` (top-level `goal [goal]` + subcommands `list/status/detail/resume/answer/stop`) @ `src/cli/program/register.goal.ts` | `command-registry.ts` L11,L156-159 | n/a | no dedicated registration test; goal commands covered by `src/commands/goal*.test.ts` and `src/goal/**` | yes — v0 keeper (`/new_goal` execution, status/list/resume/stop) | keep | `pnpm vitest run src/commands/goal*.test.ts src/goal/` |

### 9b. Lazy `entries[]` registrations (`src/cli/program/register.subclis.ts` L30-238)

Each entry below is rendered as a placeholder via `registerLazyCommand`
(`register.subclis.ts` L257-278) and replaced on first dispatch. Removing an
entry from `entries[]` deletes both the placeholder and the lazy dispatch.

| PATH (entry name @ registrar module) | IMPORTERS | PACKAGE/WORKSPACE/VITEST REFS | TESTS | REQUIRED BY v0? | DECISION | VERIFICATION NEEDED |
| --- | --- | --- | --- | --- | --- | --- |
| `acp` @ `src/cli/acp-cli.ts` (`registerAcpCli` → `serveAcpGateway`, `runAcpClientInteractive` from `src/acp/server.ts`, `src/acp/client.ts`) | `register.subclis.ts` L31-38 | `package.json` `files[]` ships `dist/cli/**` and `dist/acp/**` (if present) | `src/cli/program/register.subclis.test.ts` L4-21,L44-95 (full mocked acp dispatch test — would need updating in Track H) | no — Agent Control Protocol is not part of v0 (task delete list: `ACP`) | delete-now (drop entry from `entries[]` and update `register.subclis.test.ts` in same Track H commit) | `pnpm vitest run src/cli/program/register.subclis.test.ts` |
| `gateway` @ `src/cli/gateway-cli.ts` (`registerGatewayCli`) | `register.subclis.ts` L39-46; help `EXAMPLES` L16-19 references `moltbot gateway`, `moltbot --dev gateway`, `moltbot gateway --force` | `package.json` `files[]` ships `dist/cli/**`; `src/cli/gateway-cli.coverage.test.ts`, `src/cli/gateway.sigterm.test.ts` | `src/cli/gateway-cli.coverage.test.ts`, `src/cli/gateway.sigterm.test.ts`, `src/cli/program.force.test.ts` | yes — v0 keeper | keep | `pnpm vitest run src/cli/gateway*` |
| `daemon` @ `src/cli/daemon-cli.ts` (`registerDaemonCli`; re-exported runners include `runDaemonInstall/Restart/Start/Status/Stop/Uninstall`) | `register.subclis.ts` L47-54 ("Gateway service (legacy alias)") | `src/cli/daemon-cli.coverage.test.ts` | `src/cli/daemon-cli.coverage.test.ts` | partial — task delete list mentions `daemon/deploy`, but `daemon` here is the legacy alias for the v0 gateway service (`gateway install/start/stop`); `runDaemonInstall/Restart/Start/Status/Stop/Uninstall` are still imported by deploy-adjacent installers | quarantine-or-defer (hide from default surface in Track H by commenting registration with the Stage 2G note; defer file deletion until install/restart consumers are re-pointed onto `gateway`) | `pnpm vitest run src/cli/daemon-cli*` |
| `logs` @ `src/cli/logs-cli.ts` (`registerLogsCli`) | `register.subclis.ts` L55-62 | `src/cli/logs-cli.test.ts` | `src/cli/logs-cli.test.ts` | yes — v0 keeper | keep | `pnpm vitest run src/cli/logs-cli.test.ts` |
| `system` @ `src/cli/system-cli.ts` (`registerSystemCli`; system events / heartbeat / presence via `addGatewayClientOptions`/`callGatewayFromCli`) | `register.subclis.ts` L63-70 | n/a | no dedicated CLI test (system-cli has none under `src/cli/`) | yes — v0 keeper | keep | `pnpm exec tsc`, `pnpm build` |
| `models` @ `src/cli/models-cli.ts` (`registerModelsCli`) | `register.subclis.ts` L71-78 | `src/cli/models-cli.test.ts` | `src/cli/models-cli.test.ts` | yes — v0 keeper | keep | `pnpm vitest run src/cli/models-cli.test.ts` |
| `approvals` @ `src/cli/exec-approvals-cli.ts` (`registerExecApprovalsCli`) | `register.subclis.ts` L79-86 | `src/cli/exec-approvals-cli.test.ts` | `src/cli/exec-approvals-cli.test.ts` | yes — v0 keeper | keep | `pnpm vitest run src/cli/exec-approvals-cli.test.ts` |
| `nodes` @ `src/cli/nodes-cli.ts` → `src/cli/nodes-cli/register.ts` (fans out to `register.status/pairing/invoke/notify/canvas/camera/screen/location`) | `register.subclis.ts` L87-94; `register.subclis.test.ts` L12-19,L68-80 (mocked `nodes list` dispatch) | `src/cli/nodes-cli.coverage.test.ts`, `src/cli/nodes-camera.test.ts`, `src/cli/nodes-canvas.test.ts`, `src/cli/nodes-screen.test.ts`, `src/cli/program.nodes-basic.test.ts`, `src/cli/program.nodes-media.test.ts` | as above | no — `nodes` is the gateway-owned node pairing + canvas/camera/screen/location subsurface, not part of v0 Telegram/goal/repo-chat (task delete list: `node/canvas`) | quarantine-or-defer (hide from default CLI surface; deletion paired with Track I canvas-host / Track G browser/UI decision since `register.canvas.ts` imports `nodes-cli/a2ui-jsonl.ts` and node-host runner ties into the same canvas-host subsystem) | `pnpm vitest run src/cli/program.nodes-* src/cli/nodes-* src/cli/program/register.subclis.test.ts` |
| `devices` @ `src/cli/devices-cli.ts` (`registerDevicesCli`; device pairing + scoped token management via Gateway RPC) | `register.subclis.ts` L95-102 | `package.json` `files[]` ships `dist/cli/**` | none direct under `src/cli/`; device flows covered indirectly by `src/pairing/**` tests | no — device pairing/token surface is a non-v0 channel/device administration tool | quarantine-or-defer (hide registration; deletion paired with Track F pairing/channels decisions) | `pnpm exec tsc`, `pnpm build` |
| `node` @ `src/cli/node-cli.ts` → `src/cli/node-cli/register.ts` (top-level `node` with `run`/daemon `install/start/stop/restart/status/uninstall`; runs headless node host via `src/node-host/runner.ts`, `src/node-host/config.ts`) | `register.subclis.ts` L103-110 | `package.json` `files[]` ships `dist/cli/**` and `dist/node-host/**` | no direct CLI test; `src/node-host/**` tests cover the runner | no — headless node host is the same node/canvas subsurface (task delete list: `node/canvas`) | quarantine-or-defer (hide registration; deletion paired with Track I canvas-host decision because `src/node-host/runner.ts` imports `src/browser/` and `src/canvas-host/`) | `pnpm vitest run src/node-host/` |
| `sandbox` @ `src/cli/sandbox-cli.ts` (`registerSandboxCli`; `sandbox list/recreate/explain` via `commands/sandbox.ts` + `commands/sandbox-explain.ts`) | `register.subclis.ts` L111-118 | `package.json` `files[]` ships `dist/cli/**` and `dist/commands/**`; Dockerfile.sandbox + sandbox shell scripts paired (section 7) | `src/commands/sandbox*.test.ts` | no — sandbox tools manage Docker sandboxes (non-v0; section 7 deploy decision is defer for the sandbox stack) | quarantine-or-defer (hide registration; pair deletion with section 7 sandbox Dockerfile decision) | `pnpm vitest run src/commands/sandbox*.test.ts` |
| `gstack` @ `src/cli/gstack-cli.ts` (`registerGstackCli`; thin pass-through to `commands/gstack-runner.ts` → `runGstack` launching Claude Code with goal permissions) | `register.subclis.ts` L119-126; help `EXAMPLES` does not list `gstack` | `package.json` `files[]` ships `dist/cli/**`; no dedicated test under `src/cli/` | no direct CLI test | partial — `gstack` is the operator-facing launcher for the same Claude Code permission stack the goal workers use; not directly Telegram/goal/repo-chat, but useful for goal debugging | defer (low-risk to keep; revisit if CLI surface needs further narrowing) | `pnpm exec tsc`, `pnpm build` |
| `tui` @ `src/cli/tui-cli.ts` (`registerTuiCli` → `runTui` from `src/tui/tui.ts`) | `register.subclis.ts` L127-134 | `package.json` `files[]` ships `dist/cli/**` and `dist/tui/**` | `src/cli/program.smoke.test.ts` L42 (`vi.mock("../tui/tui.js", () => ({ runTui }))`), L104-123 (`runs tui ...`, `warns and ignores invalid tui timeout override`) | no — terminal UI is not part of v0 (task delete list: `TUI`) | delete-now (drop `tui` entry from `entries[]`; update the three `runs tui ...` cases in `program.smoke.test.ts` in the same Track H commit; also unwire `vi.mock("../tui/tui.js")` if no other test consumes it) | `pnpm vitest run src/cli/program.smoke.test.ts src/cli/program/register.subclis.test.ts` |
| `cron` @ `src/cli/cron-cli.ts` (`registerCronCli` → `src/cli/cron-cli/`) | `register.subclis.ts` L135-142 | `src/cli/cron-cli.test.ts` | `src/cli/cron-cli.test.ts` | yes — v0 keeper | keep | `pnpm vitest run src/cli/cron-cli.test.ts` |
| `dns` @ `src/cli/dns-cli.ts` (`registerDnsCli`; wide-area DNS / tailnet helpers via `pickPrimaryTailnetIPv4`, `getWideAreaZonePath`) | `register.subclis.ts` L143-150 | `src/cli/dns-cli.test.ts` | `src/cli/dns-cli.test.ts` | no — DNS/tailnet helpers are tied to the deploy/tailnet surface scoped out by section 7 | quarantine-or-defer (hide registration; pair deletion with Tailscale/widearea infra decision) | `pnpm vitest run src/cli/dns-cli.test.ts` |
| `docs` @ `src/cli/docs-cli.ts` (`registerDocsCli` → `docsSearchCommand` from `commands/docs.ts`) | `register.subclis.ts` L151-158 | `package.json` `files[]` ships `dist/cli/**` and `dist/commands/**` | none direct under `src/cli/`; covered by `src/commands/docs*.test.ts` if any | no — docs search is not part of v0 (task delete list: `docs`) | delete-now (drop `docs` entry from `entries[]`; if `src/commands/docs.ts` has no other importer in Track H, schedule for follow-up deletion) | `pnpm vitest run src/cli/program/register.subclis.test.ts` |
| `hooks` @ `src/cli/hooks-cli.ts` (`registerHooksCli`; lists/installs hook plugins, status/manage) | `register.subclis.ts` L159-166 | `src/cli/hooks-cli.test.ts` | `src/cli/hooks-cli.test.ts` | no — public hooks CLI is not part of v0 (task delete list: `hooks/webhooks`; goal lessons / memory remain via `memory` CLI) | delete-now (drop `hooks` entry from `entries[]`; update `hooks-cli.test.ts` or prune it in same Track H commit) | `pnpm vitest run src/cli/hooks-cli.test.ts src/cli/program/register.subclis.test.ts` |
| `webhooks` @ `src/cli/webhooks-cli.ts` (`registerWebhooksCli`; Gmail-only `runGmailService`/`runGmailSetup`) | `register.subclis.ts` L167-174 | `package.json` `files[]` ships `dist/cli/**` | no direct CLI test; covered indirectly by `src/hooks/gmail*` tests | no — Gmail webhooks are not part of v0 (task delete list: `hooks/webhooks`) | delete-now (drop `webhooks` entry from `entries[]`; pair deletion of `src/hooks/gmail*` with section 6 hook decisions) | `pnpm vitest run src/cli/program/register.subclis.test.ts` |
| `pairing` @ `src/cli/pairing-cli.ts` (`registerPairingCli`; also eagerly calls `registerPluginCliCommands` with config before mounting pairing) | `register.subclis.ts` L175-187 | `src/cli/pairing-cli.test.ts` | `src/cli/pairing-cli.test.ts` | no — pairing CLI is the channel-plugin pairing-code surface (Discord/Slack/Matrix/etc); not v0 | quarantine-or-defer (hide registration; pair deletion with section 4 extensions decisions) | `pnpm vitest run src/cli/pairing-cli.test.ts` |
| `plugins` @ `src/cli/plugins-cli.ts` (`registerPluginsCli`; install/list/remove/update plugins + `registerPluginCliCommands`) | `register.subclis.ts` L188-197 | `package.json` `files[]` ships `dist/cli/**` and `dist/plugins/**` | covered indirectly by `src/plugins/**` tests | no — plugin management is the channel-plugin admin surface (task delete list: `plugins/channels`) | delete-now (drop `plugins` entry from `entries[]`; defer deletion of `src/plugins/cli.ts` until section 4 extensions decision lands) | `pnpm vitest run src/plugins/` |
| `channels` @ `src/cli/channels-cli.ts` (`registerChannelsCli`; `channels add/list/remove/status/logs/capabilities/resolve` + `channels login/logout` via `runChannelLogin/Logout`) | `register.subclis.ts` L198-205; `command-registry.ts` route smoke L205-222 in `program.smoke.test.ts` (`runs channels login`, `runs channels logout`) | `package.json` `files[]` ships `dist/cli/**` | `src/cli/program.smoke.test.ts` L41 (`vi.mock("./channel-auth.js", () => ({ runChannelLogin, runChannelLogout }))`), L205-222 | no — channel management is the non-Telegram channel-admin surface (task delete list: `plugins/channels`) | delete-now (drop `channels` entry from `entries[]`; prune `runs channels login/logout` cases + `vi.mock` in `program.smoke.test.ts` same Track H commit) | `pnpm vitest run src/cli/program.smoke.test.ts src/cli/program/register.subclis.test.ts` |
| `directory` @ `src/cli/directory-cli.ts` (`registerDirectoryCli`; channel-directory listing via `resolveChannelDefaultAccountId`/`getChannelPlugin`) | `register.subclis.ts` L206-213 | `package.json` `files[]` ships `dist/cli/**` | none direct under `src/cli/` | no — channel directory CLI is paired with `channels`/`plugins` (non-v0) | delete-now (drop `directory` entry from `entries[]` alongside `channels`/`plugins` deletions) | `pnpm vitest run src/cli/program/register.subclis.test.ts` |
| `security` @ `src/cli/security-cli.ts` (`registerSecurityCli`; `runSecurityAudit` + `fixSecurityFootguns`) | `register.subclis.ts` L214-221 | `package.json` `files[]` ships `dist/cli/**` and `dist/security/**` | covered indirectly by `src/security/**` tests | no — security audit/fix is a generic operator tool, not part of v0 Telegram/goal/repo-chat surface; `src/security/audit*.ts` is still imported by `src/cli/browser-cli-*.ts` (paired with browser deletion) | quarantine-or-defer (hide registration; pair deletion with Track I browser/security decisions) | `pnpm vitest run src/security/` |
| `skills` @ `src/cli/skills-cli.ts` (`registerSkillsCli`) | `register.subclis.ts` L222-229 | `src/cli/skills-cli.test.ts` | `src/cli/skills-cli.test.ts` | yes — v0 keeper (note: Track G prunes the `skills/peekaboo` soft-skip case at L252 of this test) | keep | `pnpm vitest run src/cli/skills-cli.test.ts` |
| `update` @ `src/cli/update-cli.ts` (`registerUpdateCli`; also reachable via the `--update` flag rewrite in `src/cli/run-main.ts`) | `register.subclis.ts` L230-237; `src/cli/run-main.ts` (`rewriteUpdateFlagArgv`) | `src/cli/update-cli.test.ts`, `src/cli/run-main.test.ts` | `src/cli/update-cli.test.ts`, `src/cli/run-main.test.ts` (full suite of `--update` argv rewrite cases) | partial — CLI self-update is a developer/operator support tool, not part of the v0 Telegram/goal product surface, but commonly relied on for local CLI usage (`local CLI as a support/debug path` per task brief) | defer (keep on the public surface for now; revisit if package surface needs further narrowing) | `pnpm vitest run src/cli/update-cli.test.ts src/cli/run-main.test.ts` |

### 9c. Help enumeration (`src/cli/program/help.ts` `EXAMPLES`)

The `EXAMPLES` array (`help.ts` L10-28) is rendered by the `afterAll` help hook
(L83-87). It is not a command registration but it leaks command names into the
public surface that Track H will reduce; deletions in 9b must drop the matching
example rows in the same commit.

| EXAMPLE LINE (L#) | COMMAND REFERENCED | DECISION |
| --- | --- | --- |
| L11 | `moltbot channels login --verbose` | drop in Track H (paired with `channels` delete-now) |
| L12-14 | `moltbot message send --target +15555550123 --message "Hi" --json` | drop in Track H (paired with `message` quarantine-or-defer) |
| L15 | `moltbot gateway --port 18789` | keep |
| L16 | `moltbot --dev gateway` | keep |
| L17 | `moltbot gateway --force` | keep |
| L18 | `moltbot gateway ...` | keep |
| L19-22 | `moltbot agent --to +15555550123 --message "Run summary" --deliver` | keep (v0 keeper) |
| L23-26 | `moltbot message send --channel telegram --target @mychat --message "Hi"` | rewrite or drop in Track H (Telegram-only — could be rewritten to a Telegram-only example if `message` is retained as a Telegram send shim; otherwise drop with the rest of `message`) |

### 9d. Test-surface footprint summary

Tests that will need an edit when Track H removes the entries marked delete-now
above:

- `src/cli/program/register.subclis.test.ts` — currently mocks/asserts `acp`
  (L4-21, L44-95) and `nodes` (L12-19, L68-80). If `acp` is delete-now and
  `nodes` is quarantine-or-defer, this file must be updated to mock surviving
  entries (e.g. `gateway`) or its assertions narrowed.
- `src/cli/program.smoke.test.ts` — currently asserts `message send/react`
  (L62-90), `tui ...` (L104-123), and `channels login/logout` (L205-222). All
  three are delete-now or quarantine-or-defer; the relevant cases plus the
  `vi.mock("../tui/tui.js")` (L42) and `vi.mock("./channel-auth.js")` (L41)
  hoists must be removed or rescoped in the same Track H commit. Cases that
  exercise v0 keepers (`status`, `memory`, `config`, `setup`, `onboard`,
  `passes auth api keys to onboard`) stay as-is.
- `src/cli/run-main.test.ts` — covers only the `--update` flag rewriter and is
  paired with the `update` entry decision (defer); no edit needed in Track H.

### 9e. Stop conditions for Track H

- Do not delete `daemon`, `nodes`, `node`, `sandbox`, or `security` entries
  without first resolving their paired subsystem decisions (gateway service
  installers / canvas-host / Docker sandbox / browser-cli security audit) —
  these are explicitly quarantine-or-defer above.
- Do not delete `browser` registration without also dropping or pruning the
  `src/cli/browser-cli*.test.ts` slice in the same Track H commit (paired with
  Track I `src/browser/` decision).
- Do not delete `message` entry wholesale — Track K legacy auto-reply triage
  and `src/auto-reply/` decisions must land first (constraint forbids broad
  `src/auto-reply/` deletion in Stage 2G).
